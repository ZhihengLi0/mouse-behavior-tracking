...
"""Selection figure with the keypoint time series underneath (advisor request, 2026-09-20).
    ...
    python viz_selection_timeline.py --batch-no N        # N = 1..5, one figure per batch
...
sys.path.insert(0, str(UNIT.parent / "time-series-analysis" / "scripts"))
from pupil_trace import load, pupil_trace  # noqa: E402

EPS_FRAC = 0.03                       # same jump threshold as select_scale_frames.py
PRED = {2: ("predictions_step01", "model trained on 20 Pluto labels (mAP-best epoch 70)"),
        3: ("predictions_step02", "model trained on 40 Pluto labels (final snapshot)"),
        4: ("predictions_step03", "model trained on 60 Pluto labels (final snapshot)"),
        5: ("predictions_step04", "model trained on 80 Pluto labels (final snapshot)")}
...
if a.batch_no == 1:
    src = sorted(PRED)[-1]
    folder, who = PRED[src]
    who = f"{who} - batch 1 itself used NO model, traces shown for orientation only"
else:
    folder, who = PRED[a.batch_no]
d = load(sorted((TD / folder).glob("*.h5"))[-1])
raw, fil, _, info = pupil_trace(d)
fps = 60.0
t_all = np.arange(len(d)) / fps
pool_end = (fidx.max() + 5) / fps if a.batch_no == 1 else 57158 / fps
...
how = "k-means over the whole training pool (no model yet)" if a.batch_no == 1 else f"jump rule on the {PRED[a.batch_no][1]}, then k-means among the flagged frames"
fig.suptitle(f"{name}: how the 20 frames were chosen - {how}\nTime series below: {who}. "
             "Grey = computed on every frame, color = untrusted frames bridged (3-point endpoint pupil).", fontsize=12.5, y=0.915)
out = UNIT / "results" / f"04_{name}_selection_timeline.png"

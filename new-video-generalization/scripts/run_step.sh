...
if $PY scale_step.py --step $STEP --shuffle $SHUF >> "$LOG" 2>&1; then
  ...
  say "step done (tsi $TSI); analyzing the video with the new model"
...
deeplabcut.analyze_videos(r"/Users/lizhiheng/Desktop/生物/dlc_projects/EyePupilBlink-Zhiheng-2026-08-17/config.yaml",
    [r"$UNIT/training-data/20251031/20251031_Pluto_spont_1.mp4"], shuffle=$SHUF, trainingsetindex=$TSI,
    device="mps", batch_size=32, destfolder=r"$DEST")
PYEOF
  H5=$(ls "$DEST"/*shuffle${SHUF}*.h5 2>/dev/null | tail -1)
  if [ -n "$H5" ]; then
